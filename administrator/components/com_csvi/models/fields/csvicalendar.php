 ...
 * @since    2.0
 */
class FOFFormFieldCsviCalendar extends JFormFieldCalendar implements FOFFormField
{
	protected $static;
	...
	protected function getCalendar($display)
	{
		...
		$prettyempty  = $this->element['prettyempty'] ? (bool) $this->element['prettyempty'] : false;
		...
		if ($this->value)
		{
			...
			// Transform the date string if not empty
			if ($prettyempty)
			{
				if (substr($this->value, 0, 4) == '0000')
				{
					$this->value = JText::_('COM_CSVI_TEMPLATE_NEVER_RUN');
				}
			}

			if ($this->value != JText::_('COM_CSVI_TEMPLATE_NEVER_RUN'))
			{
