...
class Com_VirtuemartModelImportProduct extends RantaiImportEngine
{
	...
	public function loadTables()
	{
		...
		if (!in_array($this->db->getPrefix() . 'virtuemart_products_' . $language, $tables))
		{
		...
		elseif (!in_array($this->db->getPrefix() . 'virtuemart_manufacturers_' . $language, $tables))
		{
			// Set the name of the table not found
			...
			foreach ($images as $key => $image)
			{
				...
				if (!empty($image))
				{
					...
					else
					{
						...
						if (strpos($dirname, $imgpath) !== false)
						{
