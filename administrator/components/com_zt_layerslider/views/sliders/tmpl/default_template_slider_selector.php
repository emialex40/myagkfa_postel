...
$tp_template_slider = ZtSliderSlider::getThemePunchTemplateSliders();
$author_template_slider = ZtSliderSlider::getDefaultTemplateSliders();

$all_slider = $this->items;

?>
<!-- THE TEMPLATE AREA -->
<div id="template_area">
    <div id="template_header_part">
        <h2><span class="revlogo-mini" style="margin-right:15px;"></span><?php echo JText::_("COM_ZT_LAYERSLIDER_SLIDER_TEMPLATE_LIBRARY"); ?></h2>
			...
			<span style="display:table-cell;vertical-align:top">
                ...
                if(!empty($author_template_slider) && is_array($author_template_slider)){
                    foreach($author_template_slider as $name => $v){
                        ?>
                        <span data-type="temp_<?php echo $name; ?>" class="template_filter_button"><?php echo $name; ?></span>
                        <?php
                    }
                }
                ?>
                <span class="template_filter_button selected" data-type="temp_all"><?php echo JText::_("COM_ZT_LAYERSLIDER_ALL_TEMPLATES");?></span>
				<span class="template_filter_button" data-type="template_free"><?php echo JText::_("COM_ZT_LAYERSLIDER_FREE_TEMPLATES");?></span>
				<span class="template_filter_button" data-type="template_premium"><?php echo JText::_("COM_ZT_LAYERSLIDER_PREMIUM_TEMPLATES");?></span>
				<span class="template_filter_button" data-type="template_package"><?php echo JText::_("COM_ZT_LAYERSLIDER_PACKAGES");?></span>
				<span class="template_filter_button" data-type="temp_slider"><?php echo JText::_("COM_ZT_LAYERSLIDER_SLIDER");?></span>
				<span class="template_filter_button" data-type="temp_carousel"><?php echo JText::_("COM_ZT_LAYERSLIDER_CAROUSEL");?></span>
				<span class="template_filter_button" data-type="temp_hero"><?php echo JText::_("COM_ZT_LAYERSLIDER_HERO");?></span>
				<span class="template_filter_button" data-type="temp_notinstalled"><?php echo JText::_("COM_ZT_LAYERSLIDER_NOT_INSTALLED");?></span>
				<span class="template_filter_button" data-type="temp_socialmedia"><?php echo JText::_("COM_ZT_LAYERSLIDER_SOCIAL_MEDIA");?></span>
				<span class="template_filter_button" data-type="temp_postbased"><?php echo JText::_("COM_ZT_LAYERSLIDER_POST_BASED");?></span>
				<span class="template_filter_button temp_new_udpated" data-type="temp_newupdate"><?php echo JText::_("COM_ZT_LAYERSLIDER_NEW_UPDATED");?></span>
			</span>
			<span style="display:table-cell;vertical-align:top;text-align:right">
				<span class="rs-reload-shop"><i class="eg-icon-arrows-ccw"></i><?php echo JText::_("COM_ZT_LAYERSLIDER_UPDATE_LIBRARY");?></span>
			</span>
        </div>
    </div>
    ...
    <div class="revolution-template-groups">
        ...
        if(!empty($tp_template_slider)){
            foreach($tp_template_slider as $m_slider){
                ...
                $etikett_a = $m_slider['cat'] == 'Revolution Base' ? JText::_("COM_ZT_LAYERSLIDER_FREE") : JText::_("COM_ZT_LAYERSLIDER_PREMIUM");
                ...
                if(!isset($m_slider['installed'])){
                    $c_slider = new ZTSlider();
                    if (is_object($m_slider)) $m_slider = get_object_vars($m_slider);
                    ...
                    <div class="template_group_wrappers <?php echo $slidercat.$package.$slidercat_new; if(isset($m_slider['filter'])){ echo implode(' ', $m_slider['filter']); } ?>">
                        ...
                        foreach($c_slides as $key => $c_slide){
                            ...
                            $c_slide['number'] = $key;
                            $c_slide['current_version'] = ($version_installed !== '') ? $version_installed : JText::_("COM_ZT_LAYERSLIDER_N_A");
                        ...
                        <div class="template_meta_line">
                            <?php if ($isnew) { ?>
                                <span class="template_new"><?php echo JText::_("COM_ZT_LAYERSLIDER_NEW"); ?></span>
                            <?php } ?>
                            <?php if ($isupdate) { ?>
                                <span class="template_new"><?php echo JText::_("COM_ZT_LAYERSLIDER_UPDATE"); ?></span>
                            <?php } ?>
                    ...
                    <div class="template_group_wrappers <?php echo $slidercat_new.$slidercat.$package; ?> temp_notinstalled not-imported-wrapper <?php if(isset($m_slider['filter'])){ echo implode(' ', $m_slider['filter']); } ?>">
                        ...
                        <div class="template_meta_line">
                            ...
                            <?php /*if ($isupdate) { ?>
								...
								<?php }*/ ?>
                            <span class="<?php echo $slidercat;?>"><?php echo $etikett_a; ?></span>
                            <span class="template_notinstalled"><?php  echo JText::_("COM_ZT_LAYERSLIDER_NOT_INSTALLED");?></span>
                        </div>
            ...
            echo '<span style="color: #F00; font-size: 20px">No data could be retrieved from the servers. Please make sure that your website can connect to the themepunch servers';
...
<div id="dialog_import_template_slider" title="<?php echo JText::_("COM_ZT_LAYERSLIDER_IMPORT_TEMPLATE_SLIDER");?>" class="dialog_import_template_slider" style="display:none">
    <form action="index.php" enctype="multipart/form-data" method="post">
        <input type="hidden" name="option" value="com_zt_layerslider">
        <input type="hidden" name="task" value="slider.importtemplate">

        <p><?php echo JText::_("COM_ZT_LAYERSLIDER_SELECT_ZIP_FILE");//_e('Please select the corresponding zip file from the download packages import folder called', 'revslider'); ?>:</p>
        ...
        <span style="margin-top:45px;display:block"><input type="submit" class="rs-import-slider-button button-primary revblue tp-be-button" value="<?php echo JText::_("COM_ZT_LAYERSLIDER_IMPORT_TEMPLATE_SLIDER");//_e("Import Template Slider",'revslider'); ?>"></span>
        <span class="tp-clearfix"></span>
        <span style="font-weight: 700;"><?php echo JText::_("COM_ZT_LAYERSLIDER_STYLE_TEMPLATE");//_e("Note: style templates will be updated if they exist!",'revslider'); ?></span>
        <table style="display: none;">
            <tr>
                <td><?php echo JText::_("COM_ZT_LAYERSLIDER_CUSTOM_ANIMATIONS");//_e("Custom Animations:",'revslider'); ?></td>
                <td><input type="radio" name="update_animations" value="true" checked="checked"> <?php echo JText::_("COM_ZT_LAYERSLIDER_OVERWRITE");//_e("overwrite",'revslider'); ?></td>
                <td><input type="radio" name="update_animations" value="false"> <?php echo JText::_("COM_ZT_LAYERSLIDER_APPEND");//_e("append",'revslider'); ?></td>
            </tr>
            <tr>
                <td><?php echo JText::_("COM_ZT_LAYERSLIDER_CUSTOM_ANIMATIONS"); echo JText::_("COM_ZT_LAYERSLIDER_STATIC_STYLE");//_e("Static Styles:",'revslider'); ?></td>
                <td><input type="radio" name="update_static_captions" value="true"> <?php echo JText::_("COM_ZT_LAYERSLIDER_OVERWRITE");//_e("overwrite",'revslider'); ?></td>
                <td><input type="radio" name="update_static_captions" value="false"> <?php echo JText::_("COM_ZT_LAYERSLIDER_APPEND");//_e("append",'revslider'); ?></td>
                <td><input type="radio" name="update_static_captions" value="none" checked="checked"> <?php echo JText::_("COM_ZT_LAYERSLIDER_IGNORE");//_e("ignore",'revslider'); ?></td>
            </tr>
        </table>
    </form>
</div>
...
<div id="dialog_import_template_slider_from" title="<?php echo JText::_("COM_ZT_LAYERSLIDER_IMPORT_TEMPLATE_SLIDER");//_e("Import Template Slider",'revslider'); ?>" class="dialog_import_template_slider_from" style="display:none">
    <?php echo JText::_("COM_ZT_LAYERSLIDER_IMPORT_LOCAL_OR_LIVE");//_e('Import Slider from local or from ThemePunch server?', 'revslider'); ?>
    <form action="index.php" enctype="multipart/form-data" method="post" name="rs-import-template-from-server" id="rs-import-template-from-server">
        <input type="hidden" name="option" value="com_zt_layerslider">
