...
<div id="template_area">
	<div id="template_header_part">
		<h2><span class="revlogo-mini" style="margin-right:15px;"></span><?php echo JText::_('Slider Template Library', 'revslider'); ?></h2>
		
		<div id="close-template"></div>
		
		<div class="revolution-template-switcher">	
			<span style="display:table-cell;vertical-align:top">			
				<span class="template_filter_button selected" data-type="temp_all"><?php echo JText::_('All Slides', 'revslider'); ?></span>
				<span class="template_filter_button" data-type="temp_slider"><?php echo JText::_('Slider', 'revslider'); ?></span>
				<span class="template_filter_button" data-type="temp_carousel"><?php echo JText::_('Carousel', 'revslider'); ?></span>			
				<span class="template_filter_button" data-type="temp_hero"><?php echo JText::_('Hero', 'revslider'); ?></span>
				<span class="template_filter_button" data-type="template_free"><?php echo JText::_('Revolution Defaults', 'revslider'); ?></span>
				<span class="template_filter_button" data-type="template_premium"><?php echo JText::_('Premium Slider', 'revslider'); ?></span>	
				<span class="template_filter_button template_local_filter" data-type="temp_existing"><?php echo JText::_('Local Slides', 'revslider'); ?></span>			
				<span class="template_filter_button template_local_filter" data-type="temp_custom"><?php echo JText::_('User Templates', 'revslider'); ?></span>			
			</span>
			<span style="display:table-cell;vertical-align:top;text-align:right">
				<span class="rs-reload-shop"><i class="eg-icon-arrows-ccw"></i><?php echo JText::_('Update Library', 'revslider'); ?></span>
			</span>
			
		</div>
		<div class="revolution-template-subtitle"><?php echo JText::_('Add Single Slide', 'revslider'); ?></div>
	</div>
	...
	<div class="revolution-basic-templates revolution-template-groups" style="padding-top:20px">
					...
					if(is_array($m_slider['plugin_require']) && !empty($m_slider['plugin_require'])){
						foreach($m_slider['plugin_require'] as $k => $pr){
							if(!isset($plugin_list[$pr['path']])){
								if(is_plugin_active(trim($pr['path']))){
					...
					if(!empty($c_slides)){
						...
						<div style="margin-bottom:30px" class="template_group_wrappers  <?php echo $slidercat.$slidercat_new; if(isset($m_slider['filter'])){ echo implode(' ', $m_slider['filter']); } ?>">							
							...
							if(isset($m_slider['preview']) && $m_slider['preview'] !== ''){
								echo '<a class="template_slide_preview" href="'.trim($m_slider['preview']).'" target="_blank"><i class="eg-icon-search"></i></a>';
							}
							...
							foreach($c_slides as $key => $c_slide){
								...
								<div class="temp_slide_single_wrapper">
									...
									$title = str_replace("'", "", ZTSliderSlider::getVar($c_slide['params'], 'title', 'Slide'));
									...
									$c_slide['current_version'] = ($version_installed !== '') ? $version_installed : JText::_('N/A', 'revslider');
									...
									<div class="template_meta_line">
										<?php if ($isnew) { ?>
										<span class="template_new"><?php echo JText::_("New", "revslider"); ?></span>
										<?php } ?>
										<?php if ($isupdate) { ?>
										<span class="template_new"><?php echo JText::_("Update", "revslider"); ?></span>
										<?php } ?>
										<span class="<?php echo $slidercat;?>"><?php echo JText::_($etikett_a, "revslider");?></span>
										<span class="template_installed"><?php echo JText::_("Installed", "revslider"); ?><i class="eg-icon-check"></i></span>
									</div>	
							...
							foreach($c_slides as $key => $c_slide){
								...
								<?php
									...
									<div class="template_meta_line">
										...
										<?php }*/ ?>
										<span class="<?php echo $slidercat;?>"><?php echo JText::_($etikett_a, "revslider");?></span>
										<span class="template_notinstalled"><?php echo JText::_("Not Installed", "revslider"); ?></span>
									</div>	
					...
					<div class="template_group_wrappers temp_existing <?php //if(isset($c_slider['filter'])){ echo implode(' ', $c_slider['filter']); } ?>">
						...
						foreach($c_slides as $c_slide){
							...
							<?php
								...
								$title = str_replace("'", "", ZTSliderSlider::getVar($mod_slide['params'], 'title', 'Slide'));
								$tmpl->write_template_markup($mod_slide);
								?>
								<div class="template_meta_line">									
									<span class="template_local"><?php echo JText::_("Local Slide", "revslider");?></span>									
								</div>	
								<div class="template_thumb_title"><?php echo $title; ?></div>
							</div>
							<?php
		...
		<div class="template_group_wrappers temp_custom">
			...
			if(!empty($templates)){
				?>		
				<div class="template_slider_title"><?php echo JText::_('User Templates', 'revslider'); ?></div>
				<div class="temp_slides_in_slider_wrapper">					
				<?php
				foreach($templates as $template){
					...
					<div class="temp_slide_single_wrapper">
						...
						?>
						<div class="template_meta_line">									
							<span class="template_local"><?php echo JText::_("User Template", "revslider");?></span>									
						</div>	
...
<div id="dialog_import_template_slide" title="<?php echo JText::_("Import Template Slide",'revslider'); ?>" class="dialog_import_template_slide" style="display:none">
	<form action="<?php echo JRoute::_('index.php?option=com_zt_layerslider') ?>" enctype="multipart/form-data" method="post">
		<input type="hidden" name="action" value="slides.sliderajaxaction">
		...
		<p><?php echo JText::_('Please select the corresponding zip file from the download packages import folder called', 'revslider'); ?>:</p>
		...
		<span style="margin-top:45px;display:block"><input type="submit" class="rs-import-slider-button button-primary revblue tp-be-button" value="<?php echo JText::_("Import Template Slide",'revslider'); ?>"></span>
		<span class="tp-clearfix"></span>
		<span style="font-weight: 700;"><?php echo JText::_("Note: style templates will be updated if they exist!",'revslider'); ?></span><br><br>
		<table style="display: none;">
			<tr>
				<td><?php echo JText::_("Custom Animations:",'revslider'); ?></td>
				<td><input type="radio" name="update_animations" value="true" checked="checked"> <?php echo JText::_("overwrite",'revslider'); ?></td>
				<td><input type="radio" name="update_animations" value="false"> <?php echo JText::_("append",'revslider'); ?></td>
			</tr>
			<tr>
				<td><?php echo JText::_("Static Styles:",'revslider'); ?></td>
				<td><input type="radio" name="update_static_captions" value="true"> <?php echo JText::_("overwrite",'revslider'); ?></td>
				<td><input type="radio" name="update_static_captions" value="false"> <?php echo JText::_("append",'revslider'); ?></td>
				<td><input type="radio" name="update_static_captions" value="none" checked="checked"> <?php echo JText::_("ignore",'revslider'); ?></td>
			</tr>
		</table>		
...
<div id="dialog_import_template_slide_from" title="<?php echo JText::_("Import Template Slide",'revslider'); ?>" class="dialog_import_template_slide_from" style="display:none">
	<?php echo JText::_('Import Slide from local or from ThemePunch online server?', 'revslider'); ?>
	<form action="<?php echo JRoute::_('index.php?option=com_zt_layerslider') ?>" enctype="multipart/form-data" method="post" name="rs-import-slide-template-from-server" id="rs-import-slide-template-from-server">
		<input type="hidden" name="action" value="slides.sliderajaxaction">
