...
class ZTSliderOperations {
	...
	public function __construct()
	{
		...
		$this->context ="com_zt_layerslider";
	}
	...
	public static function getArrAnimations($all = true){
		...
		$arrAnimations['custom'] = array('handle' => JText::_('## Custom Animation ##'));
		$arrAnimations['v5s'] = array('handle' => '-----------------------------------');
		$arrAnimations['v5'] = array('handle' => JText::_('- VERSION 5.0 ANIMATIONS -'));
		...
		$arrAnimations['v4'] = array('handle' => JText::_('- VERSION 4.0 ANIMATIONS -'));
		...
		if($all){
			...
			$arrAnimations['vs'] = array('handle' => JText::_('- SAVED CUSTOM ANIMATIONS -'));
			$arrAnimations['vse'] = array('handle' => '--------------------------------------');
		
			//$custom = ZTSliderOperations::getCustomAnimations('customin');
			$custom = ZTSliderOperations::getCustomAnimationsFullPre('customin');
	...
	public static function getArrEndAnimations($all = true){
		$arrAnimations = array();
		$arrAnimations['custom'] = array('handle' => JText::_('## Custom Animation ##'));
		$arrAnimations['auto'] = array('handle' => JText::_('Automatic Reverse'));
		$arrAnimations['v5s'] = array('handle' => '-----------------------------------');
		$arrAnimations['v5'] = array('handle' => JText::_('- VERSION 5.0 ANIMATIONS -'));
		$arrAnimations['v5e'] = array('handle' => '-----------------------------------');
		...
		if($all){
			...
			$arrAnimations['vs'] = array('handle' => JText::_('- SAVED CUSTOM ANIMATIONS -'));
			$arrAnimations['vse'] = array('handle' => '--------------------------------------');
			//$custom = ZTSliderOperations::getCustomAnimations('customout');
			$custom = ZTSliderOperations::getCustomAnimationsFullPre('customout');
	 ...
	 */
	public function insertCustomAnim($anim){
		if(isset($anim['handle'])) {
			...
			$this->db->setQuery('INSERT INTO #__zt_layerslider_layer_animations VALUES ("'.$arrInsert['handle'].'","'.$arrInsert['params'].'","'.$arrInsert['settings'].'")');

			try{
				$result = $this->db->execute();//RevSliderGlobals::$table_layer_anims, $arrInsert
			}catch (Exception $e) {
		...
		$arrAnims['customin'] = ZTSliderOperations::getCustomAnimations();
		$arrAnims['customout'] = ZTSliderOperations::getCustomAnimations('customout');
		$arrAnims['customfull'] = ZTSliderOperations::getFullCustomAnimations();
	 ...
	 * insert custom animations
	 */
	public function updateCustomAnim($anim){
		...
		if(isset($anim['handle'])) {
			...
			$this->db->setQuery();
			$this->db->setQuery('UPDATE #__zt_layerslider_layer_animations SET params="'.$arrUpdate['params'].'" WHERE id='.(int)$id);
	 ...
	 * @since: 5.0
	 */
	public function updateCustomAnimName($anim){
		if(isset($anim['handle'])) {
			$id = $anim['id'];
			unset($anim['id']);
			$this->db->setQuery('UPDATE #__zt_layerslider_layer_animations SET handle="'.$anim['handle'].'" WHERE id='.(int)$id);
			try{
				$result = $this->db->execute();
			} catch (Exception $e) {
				$ms = $e->getMessage();
	 ...
	 */
	public function deleteCustomAnim($rawID){
		
		if(trim($rawID) != '') {
			$id = str_replace(array('customin-', 'customout-'), array('', ''), $rawID);
			$this->db->setQuery('DELETE FROM #__zt_layerslider_layer_animations WHERE id='.(int)$id);
			try{
				$this->db->execute();
	...
	public static function fillAnimations(){
		if(empty(self::$animations)){
			...
			$db->setQuery('SELECT * FROM #__zt_layerslider_layer_animations');
	...
	public static function parseCustomAnimationByArray($animArray, $is = 'start'){
		...
		if($is == 'start'){
			$retString .= 's:'.ZTSliderFunctions::getVal($animArray, 'speed', 300).';';
			$retString .= 'e:'.ZTSliderFunctions::getVal($animArray, 'easing', 'easeOutExpo').';';
		}else{
			$es = ZTSliderFunctions::getVal($animArray, 'endspeed');
			$ee = trim(ZTSliderFunctions::getVal($animArray, 'endeasing'));
			if(!empty($es)){
				$retString .= 's:'.$es.';';
	...
	public function getArrFontFamilys($slider = false){
		...
		$fonts = array(
			// GOOGLE Loaded Fonts
			array('type' => 'websafe', 'version' => JText::_('Loaded Google Fonts'), 'label' => 'Dont Show Me'),

			//Serif Fonts
			array('type' => 'websafe', 'version' => JText::_('Serif Fonts'), 'label' => 'Georgia, serif'),
			array('type' => 'websafe', 'version' => JText::_('Serif Fonts'), 'label' => '"Palatino Linotype", "Book Antiqua", Palatino, serif'),
			array('type' => 'websafe', 'version' => JText::_('Serif Fonts'), 'label' => '"Times New Roman", Times, serif'),

			//Sans-Serif Fonts
			array('type' => 'websafe', 'version' => JText::_('Sans-Serif Fonts'), 'label' => 'Arial, Helvetica, sans-serif'),
			array('type' => 'websafe', 'version' => JText::_('Sans-Serif Fonts'), 'label' => '"Arial Black", Gadget, sans-serif'),
			array('type' => 'websafe', 'version' => JText::_('Sans-Serif Fonts'), 'label' => '"Comic Sans MS", cursive, sans-serif'),
			array('type' => 'websafe', 'version' => JText::_('Sans-Serif Fonts'), 'label' => 'Impact, Charcoal, sans-serif'),
			array('type' => 'websafe', 'version' => JText::_('Sans-Serif Fonts'), 'label' => '"Lucida Sans Unicode", "Lucida Grande", sans-serif'),
			array('type' => 'websafe', 'version' => JText::_('Sans-Serif Fonts'), 'label' => 'Tahoma, Geneva, sans-serif'),
			array('type' => 'websafe', 'version' => JText::_('Sans-Serif Fonts'), 'label' => '"Trebuchet MS", Helvetica, sans-serif'),
			array('type' => 'websafe', 'version' => JText::_('Sans-Serif Fonts'), 'label' => 'Verdana, Geneva, sans-serif'),

			//Monospace Fonts
			array('type' => 'websafe', 'version' => JText::_('Monospace Fonts'), 'label' => '"Courier New", Courier, monospace'),
			array('type' => 'websafe', 'version' => JText::_('Monospace Fonts'), 'label' => '"Lucida Console", Monaco, monospace')
		);
		...
		/*if($slider !== false){
			...
			if(is_array($font_custom)){
				foreach($font_custom as $key => $curFont){
					...
					if($font != false)
						$font_custom[$key] = array('version' => JText::_('Depricated Google Fonts'), 'label' => $font);
					else
		...
		include(JPATH_ROOT.'/administrator/components/com_zt_layerslider/helpers/system/googlefonts.php');
		
		foreach($googlefonts as $f => $val){
			$fonts[] = array('type' => 'googlefont', 'version' => JText::_('Google Fonts'), 'label' => $f, 'variants' => $val['variants'], 'subsets' => $val['subsets']);
		}
	...
	private function getHtmlSelectCaptionClasses($contentCSS){
		...
		$htmlSelect = ZTSliderFunctions::getHTMLSelect($arrCaptions,"","id='layer_caption' name='layer_caption'",true);
		return($htmlSelect);
	}
	...
	public static function fillCSS(){
		if(empty(self::$css)){
			...
			$customCss = array();
			$db->setQuery('SELECT * FROM #__zt_layerslider_css');
	...
	public function getCaptionsContent(){
		...
		$result = self::$css;
		$contentCSS = ZTSliderCssParser::parseDbArrayToCss($result);
		return($contentCSS);
	}
	...
	public static function getCaptionsContentArray($handle = false){
		...
		$result = self::$css;
		$contentCSS = ZTSliderCssParser::parseDbArrayToArray($result, $handle);
		return($contentCSS);
	}
	...
	public function getStaticCss(){
		$this->db->setQuery('SELECT slider_static_css FROM #__zt_layerslider_presets');
		$css = $this->db->loadResult();

		if(!$css){
			if(file_exists(ZT_ASSETS_PATH.'/assets/css/static-captions.css')){
				$contentCSS = @file_get_contents(ZT_ASSETS_PATH.'/assets/css/static-captions.css');
				$this->updateStaticCss($contentCSS);
			}
		}
		$contentCSS = !empty($css) ? $css : '';
		return($contentCSS);
	}
	 ...
	 * get contents of the static css file
	 */
	public function updateStaticCss($content){
		$content = str_replace(array("\'", '\"', '\\\\'),array("'", '"', '\\'), trim($content));
		$this->db->setQuery('SELECT id,slider_static_css FROM #__zt_layerslider_presets');
		$c = $this->db->loadObject();
		if(is_null($c)) $this->db->setQuery('INSERT INTO #__zt_layerslider_presets VALUES (NULL ,"","","","","'.$content.'","")');
		else $this->db->setQuery('UPDATE #__zt_layerslider_presets SET slider_static_css="'.$content.'" WHERE id='.(int)$c->id);
		//
		try {
			$this->db->execute();
		} catch (Exception $e) {
	...
	public function insertCaptionsContentData($content){
		...
		$this->db->setQuery('INSERT INTO #__zt_layerslider_css VALUES ("'.$arrInsert["handle"].'","'.$arrInsert["settings"].'","'.$arrInsert["hover"].'","'.$arrInsert["params"].'","'.$arrInsert["advanced"].'")');
		try{
			$this->db->execute();
		} catch (Exception $e) {
			$ms = $e->getMessage();
		...
		$css_pasr = new ZTSliderCssParser();
		$arrCaptions = $css_pasr->get_captions_sorted();
	...
	public function updateCaptionsContentData($content){
		...
		//output captions array
		$arrCaptions = ZTSliderCssParser::get_captions_sorted();
	...
	public static function updateDynamicCaptions($full = false){
		if($full){
			$captions = array();
			$captions = ZTSliderOperations::getCaptionsContentArray();
			...
			if(!empty($classes)){
				...
				foreach($classes as $class => $val){
					$captionCheck = ZTSliderOperations::getCaptionsContentArray($class);
	...
	public static function getCaptionsCssContentArray(){
		if(file_exists(RS_PLUGIN_PATH.'public/assets/css/captions.css'))
			...
			ZTSliderFunctions::throwError(JText::_('COM_ZT_LAYERSLIDER_NO_CAPTON_CSS_FOUND'));//"No captions.css found! This installation is incorrect, please make sure to reupload the Slider Revolution plugin and try again!"
		
		$result = ZTSliderCssParser::parseCssToArray($contentCSS);
	...
	public static function importCaptionsCssContentArray(){
		...
		if(!empty($static)){ //save static into static-captions.css
			$css = ZTSliderCssParser::parseStaticArrayToCss($static);
			$static_cur = ZTSliderOperations::getStaticCss(); //get the open sans line!
			$css = $static_cur."\n".$css;
...
//		$wpmlActive = false;
		...
		//put the output html
		$urlPlugin = ZT_ASSETS_URL.'/assets/';
		$urlPreviewPattern = JUri::root()."administrator/index.php?option=com_zt_layerslider&task=slider.previewslider&sliderid=".$sliderID;
		...
		?>
			<html>
				<head>
					...
					<link rel='stylesheet' href='<?php echo $urlPlugin; ?>fonts/font-awesome/css/font-awesome.css' type='text/css' media='all' />
					<link rel='stylesheet' href='<?php echo $urlPlugin; ?>fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.css' type='text/css' media='all' />
					<?php
					...
					$http = (ZTSliderFunctions::is_ssl()) ? 'https' : 'http';
					...
					echo '<style type="text/css">'.ZTSliderCssParser::compress_css($custom_css).'</style>';
					?>
...
//					do_action('revslider_preview_slider_head');
	...
	public function previewOutputMarkup($sliderID){
		...
		if(file_exists(JPATH_ROOT.ZtSliderSlider::UPLOADSURLEXPORTZIP)){
			@unlink(JPATH_ROOT.ZtSliderSlider::UPLOADSURLEXPORTZIP); //delete file to start with a fresh one
		}
		
		$usepcl = false;
		if(class_exists('ZipArchive')){
			$zip = new ZipArchive;
			$success = $zip->open(JPATH_ROOT.ZtSliderSlider::UPLOADSURLEXPORTZIP, ZIPARCHIVE::CREATE | ZipArchive::OVERWRITE);
			
			if($success !== true){
				echo JText::_("No write permissions. Can't create zip file: ").JPATH_ROOT.ZtSliderSlider::UPLOADSURLEXPORTZIP;
				exit;
			}
		}
	...
	<head>
		...
		if($export_real){ 
			...
			if($static_css !== ''){
				...
				echo ZTSliderCssParser::compress_css($static_css);
				echo '</style>'."\n";
			}
		}else{
			?>
...
ob_end_clean();
		...
		$upload_dir = JPATH_ROOT.ZtSliderSlider::UPLOADPATH;
		$cont_url = JUri::root().'images/layerslider';
		$cont_url_no_www = str_replace('www.', '', JUri::root().'images/layerslider');
		$upload_dir_multisiteless = JPATH_ROOT.ZtSliderSlider::UPLOADPATH.'/layerslider';
		
		$search = array($cont_url, $cont_url_no_www, ZT_ASSETS_URL);
			...
			if(!empty($_files) && isset($_files[0]) && !empty($_files[0])){
				...
				foreach($_files[0] as $_file){
					...
					if(is_file($upload_dir.$_file)){
						...
						$remove = true;
					}elseif(is_file(ZT_ASSETS_URL.$_file)){
						$mf = str_replace('//', '/', ZT_ASSETS_URL.$_file);
		...
		if($export_real){ //only include if real export
			...
			if(!$usepcl){
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/extensions/revolution.extension.actions.min.js', 'js/extensions/revolution.extension.actions.min.js');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/extensions/revolution.extension.carousel.min.js', 'js/extensions/revolution.extension.carousel.min.js');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/extensions/revolution.extension.kenburn.min.js', 'js/extensions/revolution.extension.kenburn.min.js');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/extensions/revolution.extension.layeranimation.min.js', 'js/extensions/revolution.extension.layeranimation.min.js');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/extensions/revolution.extension.migration.min.js', 'js/extensions/revolution.extension.migration.min.js');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/extensions/revolution.extension.navigation.min.js', 'js/extensions/revolution.extension.navigation.min.js');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/extensions/revolution.extension.parallax.min.js', 'js/extensions/revolution.extension.parallax.min.js');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/extensions/revolution.extension.slideanims.min.js', 'js/extensions/revolution.extension.slideanims.min.js');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/extensions/revolution.extension.video.min.js', 'js/extensions/revolution.extension.video.min.js');
				
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/jquery.themepunch.enablelog.js', 'js/jquery.themepunch.enablelog.js');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/jquery.themepunch.revolution.min.js', 'js/jquery.themepunch.revolution.min.js');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/js/jquery.themepunch.tools.min.js', 'js/jquery.themepunch.tools.min.js');
				
				$zip->addFile(ZT_ASSETS_PATH.'/assets/css/settings.css', 'css/settings.css');
				
				
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.css', 'fonts/pe-icon-7-stroke/css/pe-icon-7-stroke.css');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/pe-icon-7-stroke/css/helper.css', 'fonts/pe-icon-7-stroke/css/helper.css');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.eot', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.eot');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.svg', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.svg');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.ttf', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.ttf');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.woff', 'fonts/pe-icon-7-stroke/fonts/Pe-icon-7-stroke.woff');
				
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/font-awesome/css/font-awesome.min.css', 'fonts/font-awesome/css/font-awesome.min.css');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/font-awesome/fonts/FontAwesome.otf', 'fonts/font-awesome/fonts/FontAwesome.otf');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/font-awesome/fonts/fontawesome-webfont.eot', 'fonts/font-awesome/fonts/fontawesome-webfont.eot');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/font-awesome/fonts/fontawesome-webfont.svg', 'fonts/font-awesome/fonts/fontawesome-webfont.svg');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/font-awesome/fonts/fontawesome-webfont.ttf', 'fonts/font-awesome/fonts/fontawesome-webfont.ttf');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/font-awesome/fonts/fontawesome-webfont.woff', 'fonts/font-awesome/fonts/fontawesome-webfont.woff');
				
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/revicons/revicons.eot', 'fonts/revicons/revicons.eot');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/revicons/revicons.svg', 'fonts/revicons/revicons.svg');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/revicons/revicons.ttf', 'fonts/revicons/revicons.ttf');
				$zip->addFile(ZT_ASSETS_PATH.'/assets/fonts/revicons/revicons.woff', 'fonts/revicons/revicons.woff');
			}
			
			$notice_text = "";
			$notice_text .= JText::_('Using this data is only allowed with a valid licence of the jQuery Slider Revolution Plugin, which can be found at CodeCanyon: http://codecanyon.net/item/slider-revolution-responsive-jquery-plugin/2580848?ref=themepunch');
		...
		header("Content-Disposition: attachment; filename=".trim($slider->getAlias()).".zip");
		header("Pragma: no-cache");
		header("Expires: 0");
		readfile(JPATH_ROOT.ZtSliderSlider::UPLOADSURLEXPORTZIP);
		
		@unlink(JPATH_ROOT.ZtSliderSlider::UPLOADSURLEXPORTZIP); //delete file after sending it to user
		exit();
	}
	...
	public function loadingMessageOutput(){
		?>
		<div class="message_loading_preview"><?php echo JText::_("Loading Preview...")?></div>
		<?php
	}
	...
	public function putSlidePreviewByData($data){
		...
		$data = ZTSliderFunctions::jsonDecodeFromClientSide($data);
	...
	public function updateGeneralSettings($data){
		...
		update_option('slider_global_settings', $data);
...
//		$arrValues = get_option('revslider-global-settings', '');
		$this->db->setQuery('SELECT slider_global_settings FROM #__zt_layerslider_presets');
		$arrValues = ($_r = $this->db->loadResult()) ? $_r :'';

		$arrValues = ZTSliderFunctions::maybe_unserialize ($arrValues);
	...
	/*
	 * get articles from type
	 */

	public function getArticleFromType($type,$cid = 0,$lang='*') {
		...
		switch ($type){
			case 'Categories':
				...
				$from   .= ' FROM #__content ';
				$where  .= ' WHERE catid='.$cid. ' AND state=1 AND language= "'.$lang.'"';
				break;
			case "Tags":
				$select .=' SELECT count(c.id) ';
				$from   .= ' FROM #__content as c INNER JOIN #__contentitem_tag_map as m ON c.id = m.content_item_id ';
				$where  .=' WHERE tag_id='.$cid . ' AND c.state=1 AND c.language= "'.$lang.'"';
				break;
			default:
				break;
		}
		...
		if (!empty($from)) {
			$this->db->setQuery($select.$from.$where);
	...
	public function getCatFromType($type = 'Categories',$i = 1) {
		$app = JFactory::getApplication();
		$language = $app->getUserStateFromRequest($this->context.'.filter.language', 'filter_language', '*');
		$query = '';
		switch ($type) {
			case 'Categories':
				$query .= 'SELECT id,title FROM #__categories WHERE published = 1 AND extension="com_content" AND language = "'.$language.'"';
				$value = 'category';
				break;
			case "Tags":
				$query .='SELECT id,title FROM #__tags WHERE published = 1 AND language = "'.$language.'"';
				$value = 'post_tag';
				break;
			default:
				break;
		...
		$cats_items = $this->db->loadAssocList();
		$arrCats = array();
		$arrCats["option_disabled_".$i] = "---- $type ----";
		foreach($cats_items as $key => $cat){
			$numbers = ZTSliderOperations::getArticleFromType($type,$cat['id'],$language);
			if($numbers) $arrCats[$value.$cat['id']] = $cat['title'].'('.$numbers.')';
	 ...
	 */
	public function getCatsForClient(){
		$types = array('Tags','Categories');
		$i = 1;
		$arrOutput['post'] = array();
		foreach ($types as $type) {
			$posts[$i] = $this->getCatFromType($type,$i);
			$arrOutput['post'] = array_merge($posts[$i],$arrOutput['post']);
			$i++;
		}
		return($arrOutput);
	}
	...
	public static function getCleanFontImport($font, $class = '', $url = ''){
		...
		if(strpos($font, "href=") === false){ //fallback for old versions
			$url = ZTSliderFunctions::modify_punch_url($setBase . 'fonts.googleapis.com/css?family=');
	...
	public function checkPurchaseVerification($data){
		...
		if($version_info == 'valid'){
			...
			ZTSliderFunctions::throwError(JText::_('Purchase Code already registered!'));
	...
	public static function get_slider_speed($sliderID){
		...
		$do_ssl = (ZTSliderFunctions::is_ssl()) ? 'http:' : 'https:';	
		
		$slider = new ZtSliderSlider($sliderID);
		$slider->initByID($sliderID);
...
//					}
					...
					if($urlImage !== false){
						$infos['id'] = '';
						$file = @getimagesize($slide['params']['image']) ? $slide['params']['image'] : JPATH_ROOT.'/'.$slide['params']['image'];
						$infos['info'] = pathinfo( $file );
						
						if(file_exists( $file )){
							$infos['size'] = filesize( $file );
							$infos['size-format'] = number_format($infos['size'], 2);
							$img_size += $infos['size'];
							if ($slide_counter==1)
					...
					if(strpos($slide_id, 'static_') !== false){
						$infos['url'] = JRoute::_('index.php?option=com_zt_layerslider&view=slides&id=static_'.$sliderID,false);
					}else{
						$infos['url'] = JRoute::_('index.php?option=com_zt_layerslider&view=slides&id='.$slide_id,false);
					}
				...
				if(isset($slide['layers']) && !empty($slide['layers']) && count($slide['layers']) > 0){
					...
					foreach($slide['layers'] as $lKey => $layer){
							...
							case 'image':
								...
								if(isset($layer['image_url']) && trim($layer['image_url']) != ''){
									...
									if($cur_img_id !== false){
										...
										if($urlImage !== false){
											$infos['id'] = $cur_img_id;
											$file = @getimagesize($layer['image_url'])?$layer['image_url']:JPATH_ROOT.'/'.$layer['image_url'];
											$infos['info'] = pathinfo( $file );
											if(file_exists( $file )){
												$infos['size'] = filesize( $file );
												$infos['size-format'] = number_format($infos['size'], 2);
									...
									if(strpos($slide_id, 'static_') !== false){
										...
										$infos['url'] = JRoute::_('index.php?option=com_zt_layerslider&view=slides&id='.$sliderID,false);
									}
								...
								if(!empty($poster_img)){
									foreach($poster_img as $img){
										...
										if($cur_img_id !== false){
											...
											if($urlImage !== false){
												$infos['id'] = $cur_img_id;
												$file = @getimagesize($img) ? $img : JPATH_ROOT.'/'.$img;
												$infos['info'] = pathinfo( $file );
												if(file_exists( $file )){
													$infos['size'] = filesize( $file );
													$infos['size-format'] = number_format($infos['size'], 2);
											...
											case 'html5':
												...
												if(!empty($video_arr)){
													foreach($video_arr as $type => $url){
														...
														if($cur_id !== false){
															$infos['id'] = $cur_id;
															$file = @filesize( $url ) ? $url : JPATH_ROOT.'/'.$url;
															$infos['info'] = pathinfo( $file );
															if(file_exists( $file )){
																$infos['size'] = filesize( $file );
																$infos['size-format'] = number_format($infos['size'], 2);
						...
						if(isset($layer['layer_action']) && !empty($layer['layer_action'])){
							...
							$a_action = ZTSliderFunctions::cleanStdClassToArray(ZTSliderFunctions::getVal($layer['layer_action'], 'action', array()));
							$a_link_type = ZTSliderFunctions::cleanStdClassToArray(ZTSliderFunctions::getVal($layer['layer_action'], 'link_type', array()));
		...
		<span class="tp-monitor-performance-title"><?php echo JText::_("Overall Slider Performance",'revslider'); ?></span>
		<span class="tp-monitor-performace-wrap">
			...
			<span class="tp-monitor-slow"><?php echo JText::_("Slow",'revslider'); ?></span>
			<span class="tp-monitor-ok"><?php echo JText::_("Ok",'revslider'); ?></span>
			<span class="tp-monitor-fast"><?php echo JText::_("Fast",'revslider'); ?></span>
		</span>
		...
		<span  class="tp-monitor-speed-table tp-monitor-single-speed">
			<span class="tp-monitor-speed-cell">
				<span class="tp-monitor-smalllabel"><?php echo JText::_("Load Speed UMTS:",'revslider'); ?></span>
				<span class="tp-monitor-total-subsize" id="umts-speed">%umtsspeed-single%</span>
			</span>
			<span class="tp-monitor-speed-cell">
				<span class="tp-monitor-smalllabel"><?php echo JText::_("Load Speed DSL:",'revslider'); ?></span>
				<span class="tp-monitor-total-subsize" id="dsl-speed">%dslspeed-single%</span>
			</span>
			<span class="tp-monitor-speed-cell">
				<span class="tp-monitor-smalllabel"><?php echo JText::_("Load Speed T1:",'revslider'); ?></span>
		...
		<span style="float:left;width:165px">
			<span class="tp-monitor-smalllabel"><?php echo JText::_("Total Slider Size:",'revslider'); ?></span>
		...
		<span style="float:right; width:165px">
			<span class="tp-monitor-smalllabel"><?php echo JText::_("Preloaded Slides Size:",'revslider'); ?></span>
		...
		<div id="monitor-problems" style="display:none">
			<span class="tp-monitor-performance-title"><?php echo JText::_("Need Some Attention",'revslider'); ?></span>			
			<span class="tp-clearfix" style="height:25px"></span>
		...
		if(!empty($used_images)){
			?>
			<!-- THE IMAGE PERFORMANCE MESSING -->
			<span class="tp-monitor-performance-title"><?php echo JText::_("Image Performance",'revslider'); ?></span>
			<span class="tp-monitor-performace-wrap">
			...
			<span style="float:left;width:40%">
				<span class="tp-monitor-smalllabel"><?php echo JText::_("Images Loaded:",'revslider'); ?></span>
				<span class="tp-monitor-imageicon"></span>
				<span id="image_sub_size" class="tp-monitor-total-subsize"><?php echo number_format($img_size,2); ?></span>
				</span>
			<span style="float:left;width:60%; text-align:right;">
				<span class="tp-monitor-showdetails" data-target="#monitor-image-details" data-open="</span><?php echo JText::_("Hide Details",'revslider'); ?>" data-close="</span><?php echo JText::_("Show Details",'revslider'); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo JText::_("Show Details",'revslider'); ?></span></span>
			</span>
			<span class="tp-clearfix" style="height:15px"></span>
			<!-- THE IMAGE LIST -->
				...
				foreach($used_images as $path => $image){		
					...
					if(isset($image['size'])) {
						...
						if ($image['size']>1000000) 
							$_li .=   '<span class="tp-monitor-size">'.number_format($image['size'],2).'</span>';
						else
							$_li .=   '<span class="tp-monitor-size">'.number_format($image['size'],0).'</span>';
					}else{
						if($image['id'] == 'external'){
							$_li .=   '<span class="tp-monitor-neutral"></span><span class="tp-monitor-size">'.JText::_('extern').'</span>';
						}else{
							$_li .=   '<span class="tp-monitor-warning"></span><span class="tp-monitor-size">'.JText::_('missing').'</span>';
						}
					}
		...
		?>
			...
			<span class="tp-monitor-performance-title"><?php echo JText::_("Video Performance",'revslider'); ?></span>
			<span class="tp-monitor-performace-wrap">
			...
			<span style="float:left;width:40%; display:block">				
				<span class="tp-monitor-smalllabel"><?php echo JText::_("Videos Loaded (max):",'revslider'); ?></span>
				<?php if ($video_size>0) {?>				
					<span class="tp-monitor-imageicon"></span>
					<span id="video_sub_size" class="tp-monitor-total-subsize"><?php echo number_format($video_size,2); ?></span>
				<?php } else {?>
					<span class="tp-monitor-imageicon"></span>
					<span class="tp-monitor-total-subsize"><?php echo JText::_("Unknown",'revslider'); ?></span>
				<?php } ?>
			</span>
			<span style="float:left;width:60%; text-align:right;">
				<span class="tp-monitor-showdetails" data-target="#monitor-video-details" data-open="</span><?php echo JText::_("Hide Details",'revslider'); ?>" data-close="</span><?php echo JText::_("Show Details",'revslider'); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo JText::_("Show Details",'revslider'); ?></span></span>
			</span>
			<span class="tp-clearfix" style="height:15px"></span>
			...
			foreach($used_videos as $path => $video){				
				...
				if(isset($video['size'])){
					...
					if ($video['size']>1000000) 
						$_li .= '<span class="tp-monitor-size">'.number_format($video['size'],2).'</span>';
					else
						$_li .= '<span class="tp-monitor-size">'.number_format($video['size'],0).'</span>';
				}else{
		...
		<!-- THE IMAGE PERFORMANCE MESSING -->
		<span class="tp-monitor-performance-title"><?php echo JText::_("CSS Performance",'revslider'); ?></span>
		<span class="tp-monitor-performace-wrap">
		...
		<!-- FULL SIZE OF SUBCATEGORY && SHOW/HIDE LIST -->
		<span style="float:left;width:40%">
			<span class="tp-monitor-smalllabel"><?php echo JText::_("CSS Loaded:",'revslider'); ?></span>
			<span class="tp-monitor-cssicon"></span><span id="css_sub_size" class="tp-monitor-total-subsize">%css_size%</span>
			</span>
		<span style="float:left;width:60%; text-align:right;">
			<span class="tp-monitor-showdetails" data-target="#monitor-CSS-details" data-open="</span><?php echo JText::_("Hide Details",'revslider'); ?>" data-close="</span><?php echo JText::_("Show Details",'revslider'); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo JText::_("Show Details",'revslider'); ?></span></span>
		</span>
		<span class="tp-clearfix" style="height:15px"></span>
		...
		if(file_exists( ZT_ASSETS_PATH . '/assets/css/settings.css' )){
			$fs = filesize( ZT_ASSETS_PATH . '/assets/css/settings.css' );
			$_li ='<li class="tp-monitor-listli">';
			if ($fs<60000) 
				$_li .= '<span class="tp-monitor-good"></span>';
			else
			...
			$_li .= '<span class="tp-monitor-size">'.number_format($fs,0).'</span>';			
			$_li .= '<span class="tp-monitor-file">';
			$_li .= JText::_('css/settings.css','revslider');
			$_li .= '</span>';
		...
		$_operations = new ZTSliderOperations();

		$custom_css = $_operations->getStaticCss();
		...
		$_li .= '<span class="tp-monitor-size">'.number_format(strlen($custom_css),0).'</span>';			
		$_li .= '<span class="tp-monitor-file">';
		$_li .= JText::_('Static Styles','revslider');
		$_li .= '</span>';
		...
		if(!empty($used_captions)){
			...
			$_li .=  '<span class="tp-monitor-size">'.number_format(strlen($styles),0).'</span>';			
			$_li .=  '<span class="tp-monitor-file">';
			$_li .=  JText::_('Dynamic Styles','revslider');
			$_li .=  '</span>';
		...
		echo '</ul>';			
		echo ' <span style="display:none" id="css-size-hidden">'.number_format($css_size, 2).'</span>';
		...
		<span class="tp-clearfix" style="height:15px"></span>
			...
			<span class="tp-monitor-performance-title"><?php echo JText::_("jQuery Performance",'revslider'); ?></span>
			<span class="tp-monitor-performace-wrap">
			...
			<!-- FULL SIZE OF SUBCATEGORY && SHOW/HIDE LIST -->
			<span style="float:left;width:40%; display:block">				
				<span class="tp-monitor-smalllabel"><?php echo JText::_("jQuery Loaded:",'revslider'); ?></span>				
				<span class="tp-monitor-imageicon"></span><span id="jquery_sub_size" class="tp-monitor-total-subsize">%js_size%</span>				
			</span>
			<span style="float:left;width:60%; text-align:right;">
				<span class="tp-monitor-showdetails" data-target="#monitor-jquery-details" data-open="</span><?php echo JText::_("Hide Details",'revslider'); ?>" data-close="</span><?php echo JText::_("Show Details",'revslider'); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo JText::_("Show Details",'revslider'); ?></span></span>
			</span>
			<span class="tp-clearfix" style="height:15px"></span>
		...
		$jsfiles = array(
			'jquery.themepunch.tools.min.js' => ZT_ASSETS_PATH . '/assets/js/public/jquery.themepunch.tools.min.js',
			'jquery.themepunch.revolution.min.js' => ZT_ASSETS_PATH . '/assets/js/public/jquery.themepunch.revolution.min.js',
			
		);
		//check which js files will be used by the Slider
		if($using_kenburns == true) $jsfiles['revolution.extension.kenburn.min.js'] = ZT_ASSETS_PATH . '/assets/js/public/extensions/revolution.extension.kenburn.min.js';
		if($using_parallax == true) $jsfiles['revolution.extension.parallax.js'] = ZT_ASSETS_PATH . '/assets/js/public/extensions/revolution.extension.parallax.js';
		if($using_navigation == true) $jsfiles['revolution.extension.navigation.min.js'] = ZT_ASSETS_PATH . '/assets/js/public/extensions/revolution.extension.navigation.min.js';
		if($using_videos == true) $jsfiles['revolution.extension.video.min.js'] = ZT_ASSETS_PATH . '/assets/js/public/extensions/revolution.extension.video.min.js';
		if($using_actions == true) $jsfiles['revolution.extension.actions.min.js'] = ZT_ASSETS_PATH . '/assets/js/public/extensions/revolution.extension.actions.min.js';
		if($using_layeranim == true) $jsfiles['revolution.extension.layeranimation.min.js'] = ZT_ASSETS_PATH . '/assets/js/public/extensions/revolution.extension.layeranimation.min.js';
		if($using_carousel == true)
			$jsfiles['revolution.extension.carousel.min.js'] = ZT_ASSETS_PATH . '/assets/js/public/extensions/revolution.extension.carousel.min.js';
		else
			$jsfiles['revolution.extension.slideanims.min.js'] = ZT_ASSETS_PATH . '/assets/js/public/extensions/revolution.extension.slideanims.min.js';
		...
		foreach($jsfiles as $name => $path){
			if(file_exists( $path )){
				...
				echo '<span class="tp-monitor-size">'.number_format($fs,0).'</span>';			
		...
		echo '</ul>';
		echo ' <span style="display:none" id="css-size-hidden">'.number_format($js_size, 2).'</span>';
		...
		<span class="tp-clearfix" style="height:15px"></span>
			...
			<span class="tp-monitor-performance-title"><?php echo JText::_("Google Fonts Performance",'revslider'); ?></span>
			<span class="tp-monitor-performace-wrap">
			...
			<!-- FULL SIZE OF SUBCATEGORY && SHOW/HIDE LIST -->
			<span style="float:left;width:40%; display:block">				
				<span class="tp-monitor-smalllabel"><?php echo JText::_("Fonts Loaded:",'revslider'); ?></span>				
				<span class="tp-monitor-jsicon"></span><span class="tp-monitor-total-subsize">%font_size%</span>				
			</span>
			<span style="float:left;width:60%; text-align:right;">
				<span class="tp-monitor-showdetails" data-target="#monitor-fonts-details" data-open="</span><?php echo JText::_("Hide Details",'revslider'); ?>" data-close="</span><?php echo JText::_("Show Details",'revslider'); ?>"><span class="tp-monitor-openclose"></span><span class="tp-show-inner-btn"><?php echo JText::_("Show Details",'revslider'); ?></span></span>
			</span>
			<span class="tp-clearfix" style="height:15px"></span>
		...
		if(!empty($my_fonts)){
			foreach($my_fonts as $c_font){
				$fcount = ZTSliderFunctions::get_font_weight_count($c_font);
		...
		$overall = ZTSliderOperations::get_performance($total_size/$slide_counter, 0,400000); // 400KB / Slide is ok
		$image = ZTSliderOperations::get_performance($img_size/$img_counter, 0,100000); // 100KB Image OK
		$css = ZTSliderOperations::get_performance($css_size, 0,150000); // 150KB CSS OK
		$js = ZTSliderOperations::get_performance($js_size, 0,250000); // 250KB Image OK
		$font = ZTSliderOperations::get_performance($all_font_count, 0,15); // 250KB Image OK
		...
		$content = str_replace("%overall_size%",number_format($total_size,2),$content);
		...
		$content = str_replace("%css_size%",number_format($css_size,2),$content);
		
		$content = str_replace("%js_performance%",$js["proc"],$content);
		$content = str_replace("%js_color%",$js["col"],$content);		
		$content = str_replace("%js_size%",number_format($js_size,2),$content);
		...
		$content = str_replace("%firstslide_size%", number_format($firstslide_size,2), $content);
		$content = str_replace("%smartslide_size%", number_format($smartslide_size,2), $content);
		$content = str_replace("%allslide_size%", number_format($total_size,2), $content);
	 ...
	 * @since: 5.0
	 */
	public static function get_preset_settings($attribs = null){
		/**
...
$presets[] = array (
    'settings' => array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/slideshow_auto_layout.png', 'name' => 'Slideshow-Auto', 'preset' => 'standardpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ( 'class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/slideshow_auto_layout.png', 'name' => 'Slideshow-Full-Width', 'preset' => 'standardpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/slideshow_auto_layout.png', 'name' => 'Slideshow-Full-Screen', 'preset' => 'standardpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/thumb_auto1.png', 'name' => 'Thumbs-Bottom-Auto', 'preset' => 'standardpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/thumbs_left_auto.png', 'name' => 'Thumbs-Left-Auto', 'preset' => 'standardpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/thumbs_right_auto.png', 'name' => 'Thumbs-Right-Auto', 'preset' => 'standardpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/scroll_fullscreen.png', 'name' => 'Vertical-Bullet-Full-Screen', 'preset' => 'standardpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/wide_fullscreen.png', 'name' => 'Wide-Full-Screen', 'preset' => 'heropreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/wide_fullscreen.png', 'name' => 'Wide-Full-Width', 'preset' => 'heropreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/wide_fullscreen.png', 'name' => 'Regular-Full-Screen', 'preset' => 'heropreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/wide_fullscreen.png', 'name' => 'Regular-Full-Width', 'preset' => 'heropreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/cover_carousel_thumbs.png', 'name' => 'Cover-Flow-Thumbs', 'preset' => 'carouselpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/cover_carousel_endless.png', 'name' => 'Cover-Flow-Infinite', 'preset' => 'carouselpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/flat_carousel_thumbs.png', 'name' => 'Flat-Infinite-Thumbs', 'preset' => 'carouselpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/flat_carousel.png', 'name' => 'Flat-Infinite', 'preset' => 'carouselpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/flat_carousel_thumbs_left.png', 'name' => 'Flat-Thumbs-Left', 'preset' => 'carouselpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/carousel_thumbs_right_fullscreen.png', 'name' => 'Full-Screen-Thumbs-Right', 'preset' => 'carouselpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/cover_carousel_thumbs.png', 'name' => 'Cover-Flow-Full-Screen', 'preset' => 'carouselpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
...
$presets[] = array (
    'settings' => 
    array ('class' => '', 'image' => JUri::root().'administrator/components/com_zt_layerslider/assets/images/sliderpresets/carousel_full_rounded.png', 'name' => 'Cover-Flow-Rounded', 'preset' => 'carouselpreset' ),
    'values' => 
    array (
      'next_slide_on_window_focus' => 'off',
      'delay' => '9000',
	...
	//data.layer - Layer as jQuery Object
      ...
      'custom_javascript' => '',
		...
		//add the presets made from customers --preset_default
		$db = JFactory::getDbo();
		$db->setQuery("SELECT preset_default FROM #__zt_layerslider_presets");
		$customer_presets = $db->loadResult();
		$customer_presets = !empty($customer_presets) ? json_decode($customer_presets,true) : array();
	 ...
	 * @since: 5.0
	 **/
	public function add_preset_setting($data){
		
		if(!isset($data['settings']) || !isset($data['values'])) return JText::_('COM_ZT_LAYERSLIDER_MISSING_VALUES');//Missing values to add preset

		$this->db->setQuery("SELECT id,preset_default FROM #__zt_layerslider_presets");
		$_presets = $this->db->loadObject();
		$is_null  =  !empty($_presets) ? false : true;
		if ($is_null) {
			$_presets = array();
		} else {
			$_preset_id = $_presets->id;
			$_presets = !empty($_presets->preset_default) ? json_decode($_presets->preset_default,true) : array();
		}
		...
		if (isset($_preset_id)) {
			$this->db->setQuery("UPDATE #__zt_layerslider_presets SET preset_default = '".$this->db->escape(json_encode($_presets))."' WHERE id=".(int)$_preset_id);
		} else {
			$this->db->setQuery("INSERT INTO #__zt_layerslider_presets VALUES (NULL,'','".$this->db->escape(json_encode($_presets))."','','','','')");
		}
	 ...
	 * @since: 5.0
	 **/
	public function remove_preset_setting($data){
		
		if(!isset($data['name'])) return JText::_('Missing values to remove preset');

		$this->db->setQuery("SELECT id,preset_default FROM #__zt_layerslider_presets");
		$customer_presets = $this->db->loadObject();
		if (!$customer_presets) {
			ZTSliderFunctions::ajaxResponseError(JText::_('COM_ZT_LAYERSLIDER_MISSING_ID'));
		}

		$_de_id = $customer_presets->id;

		$customer_presets = !empty($customer_presets->preset_default) ? json_decode($customer_presets->preset_default,true) : array();
		...
		if (count($customer_presets) == 0) {
			$this->db->setQuery('DELETE FROM #__zt_layerslider_presets WHERE id='.(int)$_de_id);
		} else {
			$this->db->setQuery("UPDATE  #__zt_layerslider_presets SET preset_default = '".$this->db->escape(json_encode($customer_presets))."' WHERE id=".(int)$_de_id);
		}
	 ...
	 * @since: 5.0
	 **/
	public function update_preset_setting($data){
		
		if(!isset($data['name'])) return JText::_('Missing values to update preset');

		$this->db->setQuery("SELECT id,preset_default FROM #__zt_layerslider_presets");
		$customer_presets = $this->db->loadObject();
		if(!$customer_presets or is_null($customer_presets)) {ZTSliderFunctions::ajaxResponseError(JText::_('COM_ZT_LAYERSLIDER_PRESET_EMPTY'));}
		$id = $customer_presets->id;
		$customer_presets = !empty($customer_presets->preset_default) ? json_decode($customer_presets->preset_default,true) : array();
		...
		$this->db->setQuery("UPDATE  #__zt_layerslider_presets SET preset_default = '".$this->db->escape(json_encode($customer_presets))."' WHERE id=".(int)$id);
		try{
			$this->db->execute();
		}catch (Exception $e) {
			$message = $e->getMessage();
 ...
 * @since: 5.0
 **/
class RevOperations extends ZTSliderOperations {}
?>