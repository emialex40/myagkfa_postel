...
class ZTSliderCssParser{
					...
					foreach($styles as $name => $style){
						...
						if(!is_array($name) && isset($transparency[$name])){ //the style can have transparency!
							if(isset($styles[$transparency[$name]]) && $style !== 'transparent'){
								$style = ZTSliderFunctions::hex2rgba($style, $styles[$transparency[$name]] * 100);
							}
						}
						if(!is_array($name) && isset($check_parameters[$name])){
							$style = ZTSliderFunctions::add_missing_val($style, $check_parameters[$name]);
						}
						...
						foreach($hover as $name => $style){
							...
							if(!is_array($name) && isset($transparency[$name])){ //the style can have transparency!
								if(isset($hover[$transparency[$name]]) && $style !== 'transparent'){
									$style = ZTSliderFunctions::hex2rgba($style, $hover[$transparency[$name]] * 100);
								}
							}
							if(!is_array($name) && isset($check_parameters[$name])){
								$style = ZTSliderFunctions::add_missing_val($style, $check_parameters[$name]);
	...
	public function get_captions_sorted(){
		$this->db->setQuery('SELECT * FROM #__zt_layerslider_css ORDER BY handle ASC');
		try{
 ...
 * @since: 5.0
 **/
class UniteCssParserRev extends ZTSliderCssParser {}
?>