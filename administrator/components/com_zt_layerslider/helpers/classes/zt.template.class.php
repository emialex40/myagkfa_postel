...
class ZTSliderTemplate {
				...
				if($response = $request['body']) {
					if($response !== 'invalid'){
						...
						if($ret !== false){
							...
							$return = array('error' => JText::_('Can\'t write the file into the uploads folder of WordPress, please change permissions and try again!', 'revslider'));
						}
					}else{
						$return = array('error' => JText::_('Purchase Code is invalid', 'revslider'));
					}
				}
			}//else, check for error and print it to customer
		}else{
			$return = array('error' => JText::_('Can\'t write into the uploads folder of WordPress, please change permissions and try again!', 'revslider'));
		}
			...
			if(!is_wp_error($request)) {
				if($response = maybe_unserialize($request['body'])) {
					...
					if(is_array($templates)) {
						RevSliderFunctionsWP::update_option('zt-templates-new', $templates, false);
					}
				}
			}
	...
	/**
	 * Update the Templatelist, move zt-templates-new into zt-templates
	 * @since: 5.0.5
	 */
	private function update_template_list(){

		$this->db->setQuery('SELECT zt-templates-new,zt-templates FROM #__zt_layerslider_presets');
		$result = $this->db->loadAssoc();
		$new  = !empty($result['zt-templates-new']) ? $result['zt-templates-new'] : false;
		$cur  = !empty($result['zt-templates']) ? json_decode($result['zt-templates']): array();
		...
		if($new !== false && !empty($new) && is_array($new)){
			...
			$this->db->setQuery('UPDATE #__zt_layerslider_presets SET zt-templates="'.$cur.'",zt-templates-new="false"');
			$this->db->execute();
	...
	public function remove_is_new($uid){
		$this->db->setQuery('SELECT zt-templates FROM #__zt_layerslider_presets');
		$result = $this->db->loadResult();
		$cur  = !empty($result) ? json_decode($result): array();
		...
		$this->db->setQuery('UPDATE #__zt_layerslider_presets SET zt-templates="'.$cur.'"');
		$this->db->execute();
	...
	private function _update_images(){
		$this->db->setQuery('SELECT zt-templates FROM #__zt_layerslider_presets');
		$result = $this->db->loadResult();
		$templates = !empty($result) ? json_decode($result): array();
		...
		RevSliderFunctionsWP::update_option('zt-templates', $templates, false); //remove the push_image
	}
	...
	public function copySlideToTemplates($slide_id, $slide_title, $slide_settings = array()){
		...
		$this->db->setQuery('SELECT * FROM #__zt_layerslider_slide WHERE id='.intval($slide_id));
		$duplicate = $this->db->loadAssoc();
		...
		$duplicate['ordering'] = -1;
		
		$params = json_decode($duplicate['attribs'], true);
		...
		$this->db->setQuery('INSERT INTO #__zt_layerslider_slide VALUES (NULL ,'.$duplicate["slider_id"].', '.$duplicate["ordering"].',1,"'.$this->db->escape($duplicate['attribs']).'","",1,"'.$this->db->escape($duplicate['language']).'","'.$this->db->escape($duplicate['settings']).'")');
	...
	public function getTemplateSlides(){
		...
		$this->db->setQuery('SELECT * FROM #__zt_layerslider_slide WHERE slider_id="-1"');
...
//		$templates = apply_filters('revslider_set_template_slides', $templates);
		...
		if(!empty($sliders)){
			foreach($sliders as $slider){
				...
				if(!isset($slider['installed'])){
					$this->db->getQuery('SELECT * FROM #__zt_layerslider_slide WHERE slider_id='.$slider['id']);
					$templates = array_merge($templates, $this->db->loadAssocList());
				}else{
	...
	public function getThemePunchTemplateDefaultSlides($slider_alias){
		...
		$templates = get_option('zt-templates', array());
	...
	public function getThemePunchTemplateSliders(){
		...
		//add themepunch default Sliders here
		$this->db->getQuery('SELECT * FROM #__zt_layerslider_slide WHERE type = "template"');
		$sliders = $this->db->loadAssocList();
					...
					foreach($sliders as $ikey => $installed){
						if($installed['alias'] == $slider['alias']){
							...
							if(empty($c_slides)){
								...
								$this->db->setQuery('DELETE FROM #__zt_layerslider_slide WHERE type="template" AND id='.$installed['id']);
								$this->db->execute();
								break;
							}
	...
	public function write_import_template_markup($template){
		...
		<div data-src="<?php echo $template['img']; ?>" class="template_slider_item_import"
			...
			<div class="template_preview_add_wrapper">
				...
				<a class="preview_template_slider" href="<?php echo trim($template['preview']); ?>" target="_blank"><i class="eg-icon-search"></i></a>
				<?php } ?>
		...
		<div class="template_thumb_more">
			...
			if(isset($template['setup_notes']) && !empty($template['setup_notes'])){
				?>
				<span class="ttm_label"><?php echo JText::_('Setup Notes', 'revslider'); ?></span>
				<?php
			...
			?>
			<span class="ttm_label"><?php echo JText::_('Requirements', 'revslider'); ?></span>
			<ul class="ttm_requirements">
				...
				echo JText::_('Slider Revolution Version', 'revslider');
				...
				if(isset($template['plugin_require']) && !empty($template['plugin_require'])){
					foreach($template['plugin_require'] as $pk => $pr){
						...
						if(isset($pr['url'])) echo '<a href="'.trim($pr['url']).'" target="_blank">';
						echo $pr['name'];
			...
			<span class="ttm_space"></span>
			<span class="ttm_label_direct"><?php echo JText::_('Available Version', 'revslider'); ?></span>
			<span class="ttm_label_half ttm_available"><?php echo $template['version'];?></span>	
			<span class="ttm_space"></span>
			<?php
			if($deny == '' && $allow_install == true){
				?>
				<div class="install_template_slider<?php echo $deny; ?>" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>"><i class="eg-icon-download"></i><?php echo JText::_('Install Slider', 'revslider'); ?></div>							
				<?php
				if(isset($template['package']) && $template['package'] !== ''){
					?>
					<span class="tp-clearfix" style="margin-bottom:5px"></span>
					<div class="install_template_slider_package<?php echo $deny; ?>" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>"><i class="eg-icon-download"></i><?php echo JText::_('Install Slider Pack', 'revslider'); ?></div>							
					<?php
				}
			} else {
				?>
				<div class="dontadd_template_slider_item"><i class="icon-not-registered"></i><?php echo JText::_('Requirements not met', 'revslider'); ?></div>
				<?php
			}
			?>
			<span class="tp-clearfix" style="margin-bottom:5px"></span>
	...
	public function write_import_template_markup_slide($template){
		...
		<div class="template_thumb_more">
			...
			<span class="ttm_space"></span>
			<span class="ttm_label_direct"><span class="ttm_label_half"><?php echo JText::_('Installed Vers.', 'revslider'); ?></span><span class="ttm_label_half"><?php echo JText::_('Available Vers.', 'revslider'); ?></span></span>
			<span class="ttm_label_half ttm_insalled"><span class="ttm_label_half ttm_available"><?php echo $template['version'];?></span>	
			<span class="ttm_space"></span>	
			<?php
			if($deny == '' && $allow_install == true){
				?>
				<div class="install_template_slide<?php echo $deny; ?>" data-slidenumber="<?php echo $template['number']; ?>" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>"><i class="eg-icon-download"></i><?php echo JText::_('Install Slider', 'revslider'); ?></div>							
				<?php
			} else {
				?>
				<div class="dontadd_template_slider_item"><i class="icon-not-registered"></i><?php echo JText::_('Requirements not met', 'revslider'); ?></div>
	...
	public function write_template_markup($template, $slider_id = false){
		...
		$width = ZTSliderSlider::getVar($settings, "width", 1240);
		$height = ZTSliderSlider::getVar($settings, "height", 868);
		...
		$bgPosition = ZTSliderSlider::getVar($params, "bg_position","center center");
		...
		if(isset($template['img'])){
			...
			$thumb = JUri::root().ZTSliderSlider::getVar($params, "image");
		}
		...
		if (isset($template["user_template"])) {
			...
			<div class="template_thumb_more">
				...
				if ($allow_install !== false) {
					if($slider_id !== false){
						?>								
						<div class="install_template_slider" data-zipname="<?php echo $template['zip']; ?>" data-uid="<?php echo $template['uid']; ?>"><i class="eg-icon-download"></i><?php echo JText::_('Re-Install Slider', 'revslider'); ?></div>							
						<span class="tp-clearfix" style="margin-bottom:5px"></span>
						<?php
						if(isset($template['package']) && $template['package'] !== ''){
							$txt = ($template['package_full_installded']) ? JText::_('Re-Install Slider Pack', 'revslider') : JText::_('Install Slider Pack', 'revslider');
							?>
					...
					?>
					<i class="eg-icon-plus"></i><?php if ($slider_id == false) { echo JText::_('Add Slide', 'revslider'); } else { echo JText::_('Add Slider', 'revslider'); } ?></div>	
					<?php
					if ($slider_id !== false && isset($template['package']) && $template['package'] !== '' && $template['package_full_installded']) {
						?>
						<div class="add_template_slider_item_package" data-uid="<?php echo $template['uid']; ?>"><i class="eg-icon-plus"></i><?php echo JText::_('Add Slider Pack', 'revslider'); ?></div>
						<?php
					}
				} else {
					?>
