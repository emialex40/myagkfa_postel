...
class NetpayAtol {
	...
	private $inn, $host, $email, $phone, $total;
	private $items = array();
	...
	function __construct($inn, $email, $total, $host = '', $phone = '') {		
		$this->setInn($inn);
		$this->setEmail($email);
		$this->setTotal($total);
		$this->setHost($host);
	...
	function setEmail($email) {
		...
		if (	(strlen($email) <= 64)
			&&	(	(function_exists('filter_var') // php 5.2+
	...
	function prepareShippings() {
		foreach ($this->shippings as $shipping) {
			if (empty($shipping['sum'])) {
				return true; // not add free shipping
			}
			else {
				$this->addItem($shipping['name'], $shipping['sum'], 1, $shipping['sum'], $shipping['tax'], $shipping['tax_sum']);
			}
		}
	...
	/**
	 * Get cahbox string in JSON format
	 *
