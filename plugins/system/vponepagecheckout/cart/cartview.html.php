 ...
 * $Id: cartview.html.php 106 2017-01-23 12:16:39Z abhishekdas $
 * --------------------------------------------------------------------------------------
*/
	...
	public function display($tpl = null) 
	{
		...
		switch($checkoutTask)
		{
			...
			case 'getcartlist' :
				...
				$result = array(
					'cartlist' => $html, 'pqty' => $productsCount, 
					'msg' => $messages, 'msg_type' => $message_type, 
					'selected_shipment' => $this->cart->virtuemart_shipmentmethod_id, 'selected_payment' => $this->cart->virtuemart_paymentmethod_id
				);
			...
			case 'setpayment' :
				...
				$force = !empty($data['finalise']) ? null : true;
				$app->setUserState('virtuemart.paypal.express.url', false);
				$return = $this->setPaymentMethod($saveCC, $force);
				$error = $return ? 0 : 1;
				$paymentExpresssURL = $app->getUserState('virtuemart.paypal.express.url', false);
				$skipMessages = ($payment_data && !$saveCC && $paymentExpresssURL) || $force ? true : false;
			...
			case 'setcoupon' :
				...
				if((($return == 'COM_VIRTUEMART_CART_COUPON_VALID') || (JText::_($return) == JText::_('COM_VIRTUEMART_CART_COUPON_VALID'))))
				{
					if(JPluginHelper::isEnabled('vmcoupon', 'awocoupon'))
					{
						$wait = 1;
				...
				elseif(array_key_exists($return, $warnings) || in_array(JText::_($return), $warnings))
				{
					$error = 2;
				}
		...
		switch($layoutName)
		{
			...
			default:
				...
				{
					if(is_array($this->cart->BT) && !empty($this->cart->BT['virtuemart_country_id']))
					{
						VmJsApi::JcountryStateList($this->cart->BT['virtuemart_state_id']);
					}
					
					if(is_array($this->cart->ST) && !empty($this->cart->ST['virtuemart_country_id']))
					{
