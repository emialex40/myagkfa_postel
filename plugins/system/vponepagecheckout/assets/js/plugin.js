 ...
 * $Id: plugin.js 106 2017-01-23 12:16:39Z abhishekdas $
 * --------------------------------------------------------------------------------------
 */
VPOPC = window.VPOPC || {};
...
(function($, win, doc) {
	...
	ProOPC.validatePassword = function(e) {
		...
		if (e.indexOf(' ') >= 0) {
			...
			if (VPOPC._('PASSWORD_INTEGERS') > 0 && (!i || i.length < VPOPC._('PASSWORD_INTEGERS'))) {
				...
				if (VPOPC.optioe.PASSWORD_UPPERCASE == 1) {
	...
	ProOPC.loginAjax = function() {
		...
		ProOPC.ajax('login', {
			...
			errorCallback: function() {
				$process.find('.proopc-spinner').remove();
				$btn.removeAttr('disabled');
				ProOPC.setmsg(1, 'A system error occurred during login. See console log for more details.');
			}
		});
	...
	ProOPC.getShipmentData = function() {
		var $form = $('form#proopc-shipment-form'),
		...
		ProOPC._triggerEvent(['prepare.data.shipment', $shipment[0]]);
	...
	ProOPC.inputwidth = function() {
		...
		$('.proopc-bt-address input[type="text"], .proopc-st-address input[type="text"], .proopc-additional-info input[type="text"], .proopc-confirm-order input[type="text"]').each(function() {
			$(this).css('width', '');
	...
	ProOPC.validateAllForms = function() {
		...
		if ($payments.length && !$('input[name="virtuemart_paymentmethod_id"][type="radio"]', $payments).is(':checked')) {
	...
	/*ProOPC.checkForError = function(data) {
		if (data && $.type(data) === 'object') {
			if (typeof data.error !== typeof undefined && data.error) {
				...
				var message = 'A system error was generated while processing your order request. Please try again later.';
	...
	ProOPC.commonError = function() {
		ProOPC.removePageLoader();
		ProOPC.enableSubmit();
		var message = 'A system error was generated while processing your order request. Please check console log more details.';
		ProOPC.setmsg(1, message);
	};
	...
	ProOPC.submitOrder = function(button, step) {
		...
		if (!step || step == 1) {
			...
			ProOPC.ajaxManager.addReq({
				data: 'ctask=savebtaddress&stage=final&' + $('form#EditBTAddres').serialize() + '&' + ProOPC.getToken(),
				beforeSend: function() {
					ProOPC.addPageLoader('PLG_VPONEPAGECHECKOUT_SAVING_BILLING_ADDRESS');
					ProOPC.disableSubmit();
				},
